...
class KiaApi {
  ...
  async _connectWithRefreshToken(refreshToken) {
    ...
    // Use Node's built-in https to exchange the refresh token for an access token.
    // This completely avoids bluelinky's EuropeController.login() which hits
    // /api/v1/user/signin and fails with 4010 (reCAPTCHA required).
    const tokenData = await this._httpsPost(environment.tokenUrl, environment.host, environment.basicToken, [
    ...
    // Use BlueLinky with autoLogin:false and seed the session manually
    this._client = new BlueLinky({
      username,
